...
if enable_reports:
    ...
    ai_provider = st.sidebar.selectbox(
        ...
        index=0 if AI_PROVIDER == "huggingface" else 1,
        help="Select AI provider for report generation"
    )

    if ai_provider == "huggingface":
        st.sidebar.info("💡 HF free tier has monthly credit limits. Pipeline will auto-retry on credit exhaustion.")
        ...
        vision_model = st.sidebar.selectbox(
            "Vision Model",
            ["MiniMaxAI/MiniMax-M3", "Qwen/Qwen2.5-VL-7B-Instruct", "Florence-2"],
            index=0
        )

        text_model = st.sidebar.selectbox(
            "Text Model",
            ["meta-llama/Meta-Llama-3.1-8B-Instruct", "meta-llama/Meta-Llama-3-8B-Instruct", "mistralai/Mistral-7B-Instruct-v0.3"],
            index=0
        )
    else:
...
with col2:
    ...
    if uploaded_file is not None:
        run_pipeline = st.button("🚀 Extract Keyframes", type="primary", use_container_width=True)
    ...
    if uploaded_file is not None and enable_reports and "selected" in st.session_state:
        ...
        if not api_token:
            if ai_provider == "huggingface":
                st.warning("Please enter your Hugging Face API Token in the sidebar to enable report generation.")
            else:
                st.warning("Please enter your Fireworks API Key in the sidebar to enable report generation.")
        else:
            if st.button("🚀 Generate AI Reports", type="primary", use_container_width=True):
                ...
                try:
                    ...
                    if ai_provider == "fireworks":
                        ...
                        report_status.text("🔑 Initializing Hugging Face client...")
                        report_progress.progress(5, text="Connecting to Hugging Face...")
                        from config import HF_VISION_PROVIDER, HF_TEXT_PROVIDER
                        client = HFClient(api_token, HF_VISION_PROVIDER, HF_TEXT_PROVIDER)
...
if "selected" in st.session_state:
    ...
    st.dataframe(scene_df_data, use_container_width=True)
