...
def call_single_tone(vision_summary, tone, system_prompt):
    headers = {
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": f"Video analysis:\n{vision_summary}\n\nWrite the caption based on the instructions. CRITICAL: Your caption MUST be fully standalone. Always explicitly introduce the main subject and scene (e.g. 'A woman at a desk...') in the first sentence. Do not start directly with pronouns like 'He', 'She', or 'It'."}
        ],
        "max_tokens": 150,
        "temperature": 0.6
    }
    ...
    try:
        ...
        resp.raise_for_status()
        return tone, resp.json()["choices"][0]["message"]["content"].strip()
    except Exception as e:
        print(f"[LLM Error - {tone}]: {e}")
        return tone, f"Error generating tone: {e}"

def generate_tones(vision_summary):
    if not GROQ_API_KEY:
        return {"error": "No GROQ_API_KEY"}
        
    print("[LLM] Generating 4 tones in parallel...")
    results = {}
    
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [
            executor.submit(call_single_tone, vision_summary, tone, system_prompt)
            for tone, system_prompt in TONE_PROMPTS.items()
        ]
        for future in as_completed(futures):
            tone, content = future.result()
            results[tone] = content
...
def main():
    ...
    if not os.path.exists(input_file):
        ...
        # Create a dummy input.json for the user to test
        with open(input_file, "w") as f:
            json.dump([{"task_id": "test_1", "video_url": "https://storage.googleapis.com/amd-hackathon-clips/1860079-uhd_2560_1440_25fps.mp4"}], f, indent=2)
        print(f"Created a dummy {input_file} for testing.")
